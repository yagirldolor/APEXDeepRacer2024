...
class AnalyzeTrainingProgress(GraphAnalyzer):
    ...
    def create_plot_iteration_vs_total_reward(self, axes):
        ...
        if self.episode_control.show_evaluations() and self._episode_reward_type_control.measure_total_event_rewards():
            if self._stats_control.show_median():
                self.add_line_plot_iteration_vs_evaluation_total_reward(axes, "Evaluations - Median",
                                                                        self.evaluation_phases, np.median, "C9")
            if self._stats_control.show_mean():
                self.add_line_plot_iteration_vs_evaluation_total_reward(axes, "Evaluations - Mean",
                                                                        self.evaluation_phases, np.mean, "C10")
            if self._stats_control.show_best():
                self.add_line_plot_iteration_vs_evaluation_total_reward(axes, "Evaluations - Best",
                                                                        self.evaluation_phases, np.max, "C11")
            if self._stats_control.show_worst():
                self.add_line_plot_iteration_vs_evaluation_total_reward(axes, "Evaluations - Worst",
                                                                        self.evaluation_phases, np.min, "C12")
            if self._line_fitting_control.show_scatter():
                (plot_x, plot_y) = self.get_scatter_plot_data_iteration_vs_evaluation_total_reward(
    ...
    def create_plot_iteration_vs_percent_complete(self, axes):
        ...
        if self.episode_control.show_evaluations():
            if self._stats_control.show_median():
                self.add_line_plot_iteration_vs_evaluation_percent_complete(axes, "Evaluations - Median",
                                                                            self.evaluation_phases, np.median, "C9")
            if self._stats_control.show_mean():
                self.add_line_plot_iteration_vs_evaluation_percent_complete(axes, "Evaluations - Mean",
                                                                            self.evaluation_phases, np.mean, "C10")
            if self._stats_control.show_best():
                self.add_line_plot_iteration_vs_evaluation_percent_complete(axes, "Evaluations - Best",
                                                                            self.evaluation_phases, np.max, "C11")
            if self._stats_control.show_worst():
                self.add_line_plot_iteration_vs_evaluation_percent_complete(axes, "Evaluations - Worst",
                                                                            self.evaluation_phases, np.min, "C12")
            if self._line_fitting_control.show_scatter():
                (plot_x, plot_y) = self.get_scatter_plot_data_iteration_vs_evaluation_percent_complete(
                    self.evaluation_phases)
                self.plot_scatter_data(axes, "Evaluations", plot_x, plot_y, "C3", None)
    ...
    def add_line_plot_iteration_vs_evaluation_total_reward(self, axes: Axes, label, evaluation_phases, stat_method,
                                                           colour):
        if not self._line_fitting_control.no_fitting():
            (plot_x, plot_y) = self.get_line_plot_data_iteration_vs_evaluation_total_reward(evaluation_phases,
                                                                                            stat_method)
            self.plot_line_data(axes, label, plot_x, plot_y, colour)
    ...
    def add_line_plot_iteration_vs_percent_complete(self, axes: Axes, label, episodes, stat_method, colour):
        show_final_iteration = self._final_iteration_control.show_final_iteration()
        if not self._line_fitting_control.no_fitting():
            (plot_x, plot_y) = get_line_plot_data_iteration_vs_percent_complete(episodes, stat_method,
                                                                                show_final_iteration)
            self.plot_line_data(axes, label, plot_x, plot_y, colour)

    def add_line_plot_iteration_vs_evaluation_percent_complete(self, axes: Axes, label, evaluation_phases, stat_method,
                                                               colour):
        if not self._line_fitting_control.no_fitting():
            (plot_x, plot_y) = self.get_line_plot_data_iteration_vs_evaluation_percent_complete(evaluation_phases,
                                                                                                stat_method)
            self.plot_line_data(axes, label, plot_x, plot_y, colour)
...
def get_scatter_plot_data_iteration_vs_total_reward(episodes, show_final_iteration: bool,
                                                    reward_control: EpisodeTrainingRewardTypeControl):
