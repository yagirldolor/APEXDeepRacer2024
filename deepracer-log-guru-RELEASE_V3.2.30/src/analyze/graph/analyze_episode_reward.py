...
class AnalyzeEpisodeReward(AnalyzeEpisodeStat):
    ...
    def additional_preparation_for_plots(self):
        if self._rewardTypeControl.show_reward_plus_total():
            ...
            self.reset_labels("New Reward and Future Discounted New Reward", "New Reward", "Future New Reward",
                              "Future New Reward")
        elif self._rewardTypeControl.show_all_discount_factors():
            self.reset_labels("Comparison of Future Rewards using Alternate Discount Factors", "Reward",
                              "Current Future Reward", "Future Reward")
