    ...
    def plot_rewards_per_waypoint(self, axes: Axes):
        ...
        if self.all_episodes and self._episodes_control.show_all():
            if self._stats_control.show_median():
                self.add_plot_for_rewards_per_waypoint(axes, "All - Median", self.all_episodes, np.median, "C5",
                                                       num_waypoints)
            if self._stats_control.show_mean():
                self.add_plot_for_rewards_per_waypoint(axes, "All - Mean", self.all_episodes, np.mean, "C6",
                                                       num_waypoints)
            if self._stats_control.show_best():
                self.add_plot_for_rewards_per_waypoint(axes, "All - Best", self.all_episodes, np.max, "C7",
                                                       num_waypoints)
            if self._stats_control.show_worst():
                self.add_plot_for_rewards_per_waypoint(axes, "All - Worst", self.all_episodes, np.min, "C8",
                                                       num_waypoints)
        if self.filtered_episodes and self._episodes_control.show_filtered():
            if self._stats_control.show_median():
                self.add_plot_for_rewards_per_waypoint(axes, "Filtered - Median", self.filtered_episodes, np.median,
                                                       "C1", num_waypoints)
            if self._stats_control.show_mean():
                self.add_plot_for_rewards_per_waypoint(axes, "Filtered - Mean", self.filtered_episodes, np.mean, "C2",
                                                       num_waypoints)
            if self._stats_control.show_best():
                self.add_plot_for_rewards_per_waypoint(axes, "Filtered - Best", self.filtered_episodes, np.max, "C3",
                                                       num_waypoints)
            if self._stats_control.show_worst():
                self.add_plot_for_rewards_per_waypoint(axes, "Filtered - Worst", self.filtered_episodes, np.min, "C4",
                                                       num_waypoints)
