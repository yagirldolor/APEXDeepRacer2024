...
class AnalyzeCurveFitting(TrackAnalyzer):
    ...
    def redraw(self):
        if self._chosen_point and self._chosen_bearing:
            self.track_graphics.plot_angle_line(self._chosen_point, self._chosen_bearing - 180, BACKWARDS_DISTANCE, 2,
                                                "green")
            ...
            sequences = self._all_sequences.get_matches(initial_track_speed_match, initial_slide_match,
                                                        action_speed_match, action_steering_angle_match)
    ...
    def right_button_pressed(self, chosen_point):
        ...
        if self._backwards_point:
            ...
            chose_backwards_point = backwards_distance < 0.5 * primary_distance and abs(
                primary_distance - BACKWARDS_DISTANCE) < 0.25 * BACKWARDS_DISTANCE
        ...
        if chose_backwards_point:
            ...
            self._backwards_point = geometry.get_point_at_bearing(self._chosen_point, self._chosen_bearing - 180,
                                                                  BACKWARDS_DISTANCE)
