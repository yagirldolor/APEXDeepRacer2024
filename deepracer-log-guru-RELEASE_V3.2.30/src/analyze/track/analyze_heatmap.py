...
class AnalyzeHeatmap(TrackAnalyzer):
    ...
    def __init__(self, guru_parent_redraw, track_graphics: TrackGraphics, control_frame: tk.Frame,
        ...
        self._measurement_control = MeasurementControl(self._callback_different_measurement, control_frame, False,
                                                       config_manager)
        ...
        self._appearance_control = TrackAppearanceControl(guru_parent_redraw, control_frame,
                                                          None, self._callback_quick_change_appearance,
                                                          self._callback_quick_change_appearance)
        ...
        if self._visits_heat_map:
            ...
            if self._measurement_control.measure_progress_speed() or self._measurement_control.measure_action_speed() or self._measurement_control.measure_track_speed():
                ...
                self._statistics_heat_map.draw_statistic(self.track_graphics, brightness, color_palette,
                                                         self._visits_heat_map, max_speed, min_speed)
            ...
            if not self._visits_heat_map or (
                ...
                if brightness_method:
                    ...
                    for i, e in enumerate(episodes):
                        e.apply_event_stat_to_heat_map(brightness_method, self._statistics_heat_map, skip_start,
                                                       skip_end,
                ...
                # Else still doing it the OLD kludgy way ...
                    ...
                    self._recalculate_measure_action_speed(episodes, skip_start, skip_end, action_space_filter,
                                                           waypoint_range, recalculate_visits)
                    ...
                    self._recalculate_measure_new_reward(episodes, skip_start, skip_end, action_space_filter,
                                                         waypoint_range, recalculate_visits)
                elif self._measurement_control.measure_discounted_future_reward():
                    self._recalculate_measure_discounted_future_reward(episodes, skip_start, skip_end,
                                                                       action_space_filter, waypoint_range,
                                                                       recalculate_visits)
                elif self._measurement_control.measure_new_discounted_future_reward():
                    self._recalculate_measure_new_discounted_future_reward(episodes, skip_start, skip_end,
                                                                           action_space_filter, waypoint_range,
                                                                           recalculate_visits)
                elif self._measurement_control.measure_slide():
                    ...
                    self._recalculate_measure_smoothness(episodes, skip_start, skip_end, action_space_filter,
                                                         waypoint_range, recalculate_visits)
                elif self._measurement_control.measure_acceleration():
                    self._recalculate_measure_acceleration(episodes, skip_start, skip_end, action_space_filter,
                                                           waypoint_range, recalculate_visits)
                elif self._measurement_control.measure_braking():
                    self._recalculate_measure_braking(episodes, skip_start, skip_end, action_space_filter,
                                                      waypoint_range, recalculate_visits)
                else:
    ...
    def _recalculate_measure_action_speed(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
                                          recalculate_visits: bool):
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_action_speed_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                             waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_track_speed(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_track_speed_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                            waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_progress_speed(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_progress_speed_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                               waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_reward(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_reward_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                       waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_new_reward(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_new_reward_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_discounted_future_reward(self, episodes, skip_start, skip_end, action_space_filter,
                                                      waypoint_range, recalculate_visits: bool):
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_discounted_future_reward_to_heat_map(self._statistics_heat_map, skip_start, skip_end,
                                                         action_space_filter, waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_alternate_discounted_future_reward(self, episodes, skip_start, skip_end,
                                                                action_space_filter, waypoint_range,
                                                                recalculate_visits: bool):
        e: Episode
        for i, e in enumerate(episodes):
    ...
    def _recalculate_measure_new_discounted_future_reward(self, episodes, skip_start, skip_end, action_space_filter,
                                                          waypoint_range, recalculate_visits: bool):
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_new_discounted_future_reward_to_heat_map(self._statistics_heat_map, skip_start, skip_end,
                                                             action_space_filter, waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_slide(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_slide_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                      waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_skew(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_skew_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                     waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_smoothness(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_smoothness_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_acceleration(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_acceleration_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                             waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
    ...
    def _recalculate_measure_braking(self, episodes, skip_start, skip_end, action_space_filter, waypoint_range,
        ...
        e: Episode
        for i, e in enumerate(episodes):
            e.apply_braking_to_heat_map(self._statistics_heat_map, skip_start, skip_end, action_space_filter,
                                        waypoint_range)
            if recalculate_visits:
                e.apply_visits_to_heat_map(self._visits_heat_map, skip_start, skip_end, action_space_filter,
                                           waypoint_range)
