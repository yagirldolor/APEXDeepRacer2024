...
class EpisodeFilter:
    ...
    def set_filter_complete_section_and_time(self, start_waypoint_id, finish_waypoint_id, optional_time,
                                             optional_steps):
                                            ...
                                            if self.matches_complete_section_filter(e, track):
                                                if self.matches_object_section_filter(e):
                                                    if self.matches_specific_waypoint_reward_filter(e):
                                                        if self.filter_debug_contains is None or e.does_debug_contain(
                                                                self.filter_debug_contains):
                                                            result.append(e)
        return result
            ...
            for e in episode.events:
                if e.closest_waypoint_index == self.filter_specific_waypoint_id:
                    ...
                    if self.filter_specific_waypoint_min_reward is not None and e.reward < self.filter_specific_waypoint_min_reward:
                        matches = False
                    if self.filter_specific_waypoint_min_future_reward is not None and e.discounted_future_rewards[
                        0] < self.filter_specific_waypoint_min_future_reward:
                        matches = False
