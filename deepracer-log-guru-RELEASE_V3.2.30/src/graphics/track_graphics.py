...
class TrackGraphics:
    ...
    def plot_angle_line_highlight(self, start_point, heading, distance, width, fill_colour, dash_pattern=None):
        self.angle_line_highlight_widgets.append(
            self.plot_angle_line(start_point, heading, distance, width, fill_colour, dash_pattern))
    ...
    def draw_car(self, x: float, y: float, colour: str, heading: float):
        self.car_widgets.append(
            self.plot_angled_box(x, y, config.VEHICLE_WIDTH, config.VEHICLE_LENGTH, colour, heading))
