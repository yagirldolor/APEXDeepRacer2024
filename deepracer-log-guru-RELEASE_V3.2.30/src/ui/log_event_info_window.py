...
class LogEventInfoWindow(tk.Toplevel):
    ...
    def __init__(self, parent, config_manager: ConfigManager):
        ...
        tk.Label(debug_frame, textvariable=self.debug_output, justify=tk.LEFT, font='TkFixedFont').grid(row=0, column=0,
                                                                                                        pady=5, padx=5,
                                                                                                        sticky=tk.W)
    ...
    def make_label_and_value(parent_frame, row, label, tk_variable):
        if ":" not in label:
            label += ":"
        tk.Label(parent_frame, text=label, anchor=tk.E, justify=tk.RIGHT).grid(row=row, column=0, pady=5, padx=5,
                                                                               sticky=tk.E)
    ...
    def show_event(self, event: Event, track: Track):
        ...
        self.waypoint_lap_position.set(
            str(round(track.get_waypoint_percent_from_race_start(event.closest_waypoint_index), 1)) + "  %")
